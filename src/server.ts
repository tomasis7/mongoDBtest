...
async function connectToMongo() {
  try {
    ...
    if (dbExists) {
      ...
      // Depending on the application logic, you might want to handle this differently
      // For now, we still return the client.db instance, which might create the DB on first write
    }

    return client.db(dbName); // Return the specific database needed by routes
  } catch (error) {
    console.error("Error connecting to or verifying MongoDB:", error);
    // Attempt to close the client if connection partially succeeded before error
    await client
      .close()
      .catch((closeErr) =>
        console.error(
